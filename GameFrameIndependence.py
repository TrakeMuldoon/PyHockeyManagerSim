...
def should_change(events, frame_rates, current_frame_rate):
    ...
    change = tuple(
        filter(lambda event: event.type in [pygame.KEYDOWN, pygame.MOUSEBUTTONDOWN], events)
    )
...
def main():
    ...
    while running:
        ...
        elapsed_time, color = frame_rate_independence(
            elapsed_time, delta_time, current_frame_rate, color
        )
        ...
        pygame.display.set_caption(
            f"FPS: {clock.get_fps():.2f}{' ' * 10}Animation FPS: {current_frame_rate}"
        )
